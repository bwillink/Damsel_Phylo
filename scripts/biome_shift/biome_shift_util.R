...
fig5_add_epoch_times <- function( p, max_age, dy_bars, dy_text ) {
    ...
    x_pos = max_x-c(106, 65, 56, 48, 33.9, 23, 16, 5.3, 0)
    y_pos = rep(max_y, length(x_pos))
    x_pos_mid = ( x_pos[1:(length(x_pos)-1)] + x_pos[2:length(x_pos)] ) / 2 
...
fig6_add_epoch_times <- function( p, max_age, x_offset=0, dy=0.1 ) {
    ...
    for (k in 1:length(epoch_names)) {
        #print(x_pos_mid[k])
        #print(epoch_names[k])
        p = p + annotate("text", x=-x_pos_mid[k], y=0-dy, label=epoch_names[k], hjust=0.5, size=1.5)
    }
    return(p)
...
make_graphs = function(atlas_fp, atlas_fn) {
    ...
    area_names = c( "Aus", "SE As","E As", "Eur", "Ind", "Afr", "N Am", "C Am", "S Am")
    dat = list()
...
make_pi = function(params) {
    ...
    col_names = paste("rf_simplex.",1:27,".",sep="")
...
get_state_lbl = function() {
    ...
    for (i1 in c("Trop","Warm","Cold")) {
        for (j1 in c( "Aus", "SEAs","EAs","Eur", "Ind", "Afr", "NAm","CAm","SAm")) {
            for (i2 in c("Trop","Warm","Cold")) {
                for (j2 in c("Aus", "SEAs","EAs","Eur", "Ind", "Afr", "NAm","CAm","SAm")) {
                    if (i1 != i2 && j1 != j2) {
                        next
                    } else if (i1 == i2 && j1 == j2) {
                        next
...
make_triplets = function(x, params, graph, subroot=F) {
    ...
    x_subroot = -1
    if (subroot) {
        x_subroot = sample(1:27, size=1, prob=rev_prob, replace=TRUE)
    }
...
make_trip_plot = function(dat, the_guide=NULL) {
    ...
    line_colors2 = c( "#8E24AA", "#1E88E5", "#43A047", "#FDD835", "#FB8C00", "#E53935" )
    ...
    ppb = ppb + scale_x_continuous(breaks=0:7, labels=c("Late\nCretaceous", "Paleocene","Early\nEocene","Mid/Late\nEocene","Oligocene","Early\nMiocene","Mid/Late\nMiocene","Recent"))
    ppb = ppb + ylim(0,1)
    ppb = ppb + theme_classic(base_size = 6)
...
make_event_plot = function(dat, lbl_types, line_colors, the_guide=NULL, ymax) {
    ...
    ppb = ggplot(dat, aes(color=Var2))
    ppb = ppb + geom_rect(data=dat_rect, mapping=aes(xmin=left, xmax=right, ymin=0, ymax=ymax), fill="gray92", color=NA)
    ppb = ppb + geom_segment(data=dat, mapping=aes(x=x, xend=x, y=lower80, yend=upper80), size=1.25, alpha=0.5)
    ppb = ppb + geom_segment(data=dat, mapping=aes(x=x, xend=x, y=lower95, yend=upper95), size=0.65, alpha=0.5)
    ppb = ppb + geom_line(data=dat,  mapping=aes(x=x, y=value, color=Var2), linetype=2)
    ppb = ppb + geom_point(data=dat, mapping=aes(x=x, y=value, color=Var2),size=2)
    ...
    ppb = ppb + xlab("Time interval") + ylab("Proportion of events")
    ppb = ppb + scale_x_continuous(breaks=c(0:7), labels=c("Late\nCretaceous","Paleocene","Early\nEocene","Mid/Late\nEocene","Oligocene","Early\nMiocene","Mid/Late\nMiocene","Recent"))
    ppb = ppb +  ylim(0,ymax)
    ppb = ppb + theme_classic(base_size = 6)
    ppb = ppb + theme(panel.border = element_blank(),
                      panel.grid.major = element_blank(),
...
make_state_lstt_epoch = function(df,  epoch_times, match_lbl) {
    ...
    bb1 = ret
    bbm1 = reshape2::melt(bb1$mean); bbm1$stat="mean"
    bbu951 = reshape2::melt(bb1$upper95); bbu951$stat="upper95"
    bbl951 = reshape2::melt(bb1$lower95); bbl951$stat="lower95"
    bbu801 = reshape2::melt(bb1$upper80); bbu801$stat="upper80"
    bbl801 = reshape2::melt(bb1$lower80); bbl801$stat="lower80"
